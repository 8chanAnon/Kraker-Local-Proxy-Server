...
var proxy_name = "Kraker-4b", proxy_host = "http://localhost:" + http_port + "/";
...
console.log ("=-----------------------------------------------------------------------------=");
console.log (" Kraker (version 4b) Local Proxy Server - waiting on port " + http_port + ", ctrl-C to exit ");
console.log ("=-----------------------------------------------------------------------------=");
...
function http_handler (request, response)
{
  ...
  m = param ["vpx"]; m = port ? ":@" : (m ? ":" + m : "");
  if (local > 0) n = ++reqcount; else n = 0; shadow_on = myheader ["host"];
  ...
  function create_request ()
  {
    ...
    proxy = proxy.request (options, function (res)
    {
      ...
      {
        conn.idle = true; conn.emit ("agentRemove");  // good idea or not?
        conn.timer = setTimeout (function() { conn.end(); }, 30000);
      }});
