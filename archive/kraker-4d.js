...
Improvements from version 4c to version 4d:
...
var proxy_name = "Kraker-4d", proxy_host = "http://localhost:" + http_port + "/";
...
console.log ("=-----------------------------------------------------------------------------=");
console.log (" Kraker (version 4d) Local Proxy Server - waiting on port " + http_port + ", ctrl-C to exit ");
console.log ("=-----------------------------------------------------------------------------=");
...
{
  var i, j, k, data, sub, msg = "Settings file: "; if (!this.setnames) this.setnames = "";
  ...
  if (name == settings)
  {
    ...
    sub = data.match (/\$settings=([^\$]*)/); this.setnames = sub ? "|" + sub [1] + "|" : ""; 
  }
  else if (!this.setnames.includes ("|" + name + "|")) return (msg + "NOT ALLOWED");
  ...
  for (i = 0; i < profile.length; i++)
  {
    ...
    for (j = 0; j < sub.length; j++) if (p = sub[j])
    {
      ...
      if (server.includes (p)) for (k = 0; k < server.length; k += 2)
        if (p == server[k]) { profile_count++; server.splice (k, 2); break; }

      if (r == "TOR" || r == "VPN")
      {
        r = q.substr (4); if (r && !net.isIP (r)) continue;
      }
      else if (q == "FETCH") q = ""; else
        if (q != "LOCAL" && !net.isIP (q)) continue;
...
function init_servers ()
{
  ...
  for (i = 1; i < server.length; i += 2) { j++; if (!server[i]) k++; }
  for (i = 0; i < server.length; i += 2) init_lookup (i);
...
function init_lookup (num)
{
  ...
  if (!ip) dns.resolve4 (name, function (err, addr)
  {
    if (err || !addr.length) console.log (">> DNS (failure) " + name); else
    {
...
function dns_resolve (name, vpx)
{
  ...
  if (!ip)
  {
    ...
    if (n < 0 || p == "localhost") return ("LOCAL");
  ...
  if (!vpn_host && p == "VPN") ip = ":vpn" + ip.substr (4);

  if (vpx) ip = ":VPX" + (ip [0] == ":" ? ip.substr (4) : (ip ? ":" + ip : ""));
...
function dns_lookup (addr, host, func)
{
  ...
  if (p [0] == ":" && p.length != 5) { func (p.substr (5)); return; }
  ...
  {
    p = dnslist [n + 1].split (" ");
    q = p [0]; t = this.time - (dns_count ? 300 : 0);
...
function default_handler (response, error, local)
{
  ...
  msg = "--------------------\n" +
        " Local Proxy Server \n" +
        "--------------------\n\n" +
        "Version Name: " + proxy_name + " [August 22, 2022]\n\n" +
...
function put_file (request, response, url, local)
{
  ...
  if (url [0] == "+") url = local_link (url + "?", local); else
    if (append || !url || fs.existsSync (url)) url = "";
...
{
  var data = local_data (url, ""), stat, size = 0;
  ...
  if (fs.existsSync (url)) stat = fs.statSync (url);

  if (stat) if (!stat.isDirectory()) size = stat.size; else
  {
...
function socket_pool (sock, conn, name, port, host)
{
  ...
  if (conn) // replace an existing socket (TLS upgrade)
  {
    ...
    conn.timer = sock.timer; sock.timer = null;
  }
  else
  {
    ...
    if (conn && conn.readyState != "open") { conn.idle = false; conn = null; }
...
function http_handler (request, response)
{
  ...
  var method = request.method, url = request.url, ssl = request.socket.encrypted;
  ...
  // update for v4c: convert double vertical bar to %7C
  url = url.substr (1).replace (/%7C/g, "|").replace (/\|\|/g, "%7C");
  ...
  if ((m = url.indexOf ("$")) >= 0 && (n = url.indexOf ("$", m + 1)) > 0)
  {
    ...
    if (m && m == shadow_secret) url = "\\" + n; else
      if (url [0] == "$" && !(m + shadow).includes (".")) { shadow_on = m; url = n; }
  }

  if (!(n = shadow_on)) if ((m = (n = shadow).split (".")).length == 3)
    if (m [1] == "shadow" && m [2].split (":")[0] == "localhost") n = m [0];

  if (n && !n.includes (":")) n += ":" + (ssl ? "443" : "80");
  ...
  else if ((m = shadow_host [n]) == undefined) shadow_on = ""; else
  {
  ...
  if (method == "OPTIONS" && !shadow_on && options_proc (request, response, local)) return;
  ...
  if (local < -1 || local > 1) myheader ["accept-encoding"] = "gzip";
  ...
  var config = {
    method: method, host: origin, origin: shadow_on, referral: referral, cookie: cookie, count: n,
  ...
  var options = {
    ...
    socket: conn, servername: net.isIP (head) ? "" : head
  }
  ...
  function create_request ()
  {
    ...
    proxy = proxy.request (options, function (res)
    {
      ...
      {
        conn.idle = true; conn.timer = setTimeout (function() { conn.end(); }, 30000);
      }});
    ...
    conn.idle = false; request.pipe (proxy, { end:true });
  }
...
function proc_handler (response, res, config, local)
{
  ...
  else if (local <= 0)
  {
    ...
    if (last_time <= the_time - delay)
    {
      ...
      console.log ("<Passthrough " + passthru + " - " + last_pass);
    }
  }
  ...
  if (local <= 0 || config.method == "OPTIONS") header = res.headers; else
  {
    var header_name = [
      ...
      "content-range", "accept-ranges", "access-control-allow-credentials"
    ];
  ...
  if (!(v = config.origin) || !header ["access-control-allow-credentials"]) v = "*";
  ...
  if (config.shadow != "@" && (v = res.headers [s = "location"]))
  {
    ...
    if (!config.origin) z += config.referral + config.headers; else
    {
...
function proxy_command (response, cmd, ssl)
{
  ...
  if (cmd == "flags")
  {
    ...
    // console.log(socklist.length/3); for (n = 0; n < socklist.length; n+=3)
    // console.log("?",socklist[n],socklist[n+2].idle,socklist[n+2].readyState);
  }

  if (cmd == "reload")
  {
    if (!str) str = settings; p = init_settings (str) + " (" + str + ")";
    cmd = "="; str = ""; console.log (">> " + p); add_resolver ("");
  }

  if (cmd == "activate")
  {
    cmd = "="; if (!str) add_resolver (""); p = str.split (","); str = "";
  ...
  if (cmd == "=")
  {
    ...
    for (n = 0; n < server.length; n += 2)
    {
      p = server [n + 1]; if (!p) p = "FETCH";
      str += " " + p + " " + '-'.repeat (22 - p.length) + " " + server[n] + "\n";
    }
  }
...
function proxy_handler (sock)
{
  ...
  {
    if (addr == "0.0.0.0") { socks_abort(); return; }  // version 4c
